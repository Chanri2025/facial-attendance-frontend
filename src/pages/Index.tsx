...
import Attendance from "@/Component/Attendance.tsx";
...
const FeatureCard = ({icon, title, description, onClick}: FeatureCardProps) => {
    return (
        ...
        >
            <div
                className="absolute inset-0 bg-gradient-primary opacity-0 group-hover:opacity-5 rounded-2xl transition-opacity"/>
            <div className="relative z-10">
                <div
                    className="w-16 h-16 bg-primary/10 rounded-xl flex items-center justify-center mb-4 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all">
                    {icon}
                </div>
